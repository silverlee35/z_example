 ...
 */
struct clock_mgmt_event {
	/** Type of event */
	enum clock_mgmt_event_type type;
	/** Old clock rate */
	uint32_t old_rate;
	/** New clock rate */
	uint32_t new_rate;
...
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	const struct clock_mgmt_event event = {
		.type = CLOCK_MGMT_PRE_RATE_CHANGE,
		.old_rate = old_rate,
		.new_rate = new_rate,
	};
	return clock_notify_children(clk_hw, &event);
...
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	const struct clock_mgmt_event event = {
		.type = CLOCK_MGMT_POST_RATE_CHANGE,
		.old_rate = old_rate,
		.new_rate = new_rate,
	};
	return clock_notify_children(clk_hw, &event);
...
static inline int clock_configure(const struct clk *clk_hw, const void *data)
{
	...
	LOG_MODULE_DECLARE(clock_mgmt, CONFIG_CLOCK_MGMT_LOG_LEVEL);
