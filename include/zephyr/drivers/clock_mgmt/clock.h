...
struct clk {
	 ...
	 */
	const struct clock_mgmt_driver_api *api;
...
#endif
#if defined(CONFIG_CLOCK_MGMT_CLK_NAME) || defined(__DOXYGEN__)
	/** Name of this clock */
	const char *clk_name;
#endif
};
...
#define Z_CLOCK_INIT(children_, hw_data_, api_, name_)                         \
	{                                                                      \
		IF_ENABLED(CONFIG_CLOCK_MGMT_RUNTIME, (.children = children_,))\
		IF_ENABLED(CONFIG_CLOCK_MGMT_CLK_NAME, (.clk_name = name_,))   \
