 ...
 */
typedef int (*clock_mgmt_callback_handler_t)(const struct clock_mgmt_event *ev,
					     const void *user_data);

/**
 * @typedef clock_mgmt_state_t
 * @brief Define the clock management state identifier
 */
typedef uint8_t clock_mgmt_state_t;
 ...
 * @ref CLOCK_MGMT_DEFINE_OUTPUT or @ref CLOCK_MGMT_DT_DEFINE_OUTPUT,
 * then get a reference to the output using @ref CLOCK_MGMT_GET_OUTPUT
 * or @ref CLOCK_MGMT_DT_GET_OUTPUT.
 */
struct clock_output {
#if defined(CONFIG_CLOCK_MGMT_RUNTIME) || defined(__DOXYGEN__)
	/** Clock management callback */
	struct clock_mgmt_callback *cb;
	/** Parameters of the frequency request this output has on its clock */
	struct clock_mgmt_rate_req *req;
#endif
 ...
 */
#define Z_CLOCK_MGMT_CALLBACK_NAME(symname)                                    \
 ...
 * @param symname Base symbol name for variables related to this clock output
 */
#define Z_CLOCK_MGMT_REQ_NAME(symname)                                         \
 ...
 * @param symname Base symbol name for variables related to this clock output
 */
#define Z_CLOCK_MGMT_OUTPUT_NAME(symname) _CONCAT(symname, _clock_mgmt_output)
 ...
 * @param symname Base symbol name for variables related to this clock output
 */
#define Z_CLOCK_MGMT_DEFINE_OUTPUT(node_id, secname, symname)                  \
	...
	IF_ENABLED(CONFIG_CLOCK_MGMT_RUNTIME, (                                \
	/* Clock management callback structure, stored in RAM */               \
	struct clock_mgmt_callback Z_CLOCK_MGMT_CALLBACK_NAME(symname);        \
	struct clock_mgmt_rate_req Z_CLOCK_MGMT_REQ_NAME(symname) = {          \
		.min_freq = 0U,                                                \
		.max_freq = INT32_MAX,                                         \
	};                                                                     \
	/* Define output clock structure */                                    \
	static const struct clock_output                                       \
	Z_GENERIC_SECTION(secname) Z_CLOCK_MGMT_OUTPUT_NAME(symname) = {       \
		.clk_core = CLOCK_DT_GET(node_id),                             \
		.cb = &Z_CLOCK_MGMT_CALLBACK_NAME(symname),                    \
		.req = &Z_CLOCK_MGMT_REQ_NAME(symname),                        \
	};))
 ...
 * described in devicetree should use @ref CLOCK_MGMT_DT_DEFINE_OUTPUT
 * @param node_id Node identifier for the clock node to define an output for
 * @param name Software defined name for this clock output
 */
#define CLOCK_MGMT_DEFINE_OUTPUT(node_id, name)                                \
	Z_CLOCK_MGMT_DEFINE_OUTPUT(node_id,                                    \
 ...
 */
#define CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_NAME(dev_node, name)                    \
	CLOCK_MGMT_DEFINE_OUTPUT(                                              \
 ...
 * @param name Name of the clock output
 */
#define CLOCK_MGMT_DT_INST_DEFINE_OUTPUT_BY_NAME(inst, name)                   \
	CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_NAME(DT_DRV_INST(inst), name)
 ...
 */
#define CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_IDX(dev_node, idx)                      \
	CLOCK_MGMT_DEFINE_OUTPUT(                                              \
 ...
 * @param idx Index within the "clock-outputs" property
 */
#define CLOCK_MGMT_DT_INST_DEFINE_OUTPUT_BY_IDX(inst, idx)                     \
	CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_IDX(DT_DRV_INST(inst), idx)
 ...
 * CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_IDX(dev_node, 0)
 * @param dev_node Device node with a clock-outputs property.
 */
#define CLOCK_MGMT_DT_DEFINE_OUTPUT(dev_node)                                  \
	CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_IDX(dev_node, 0)
 ...
 * @param inst DT_DRV_COMPAT instance number
 */
#define CLOCK_MGMT_DT_INST_DEFINE_OUTPUT(inst)                                 \
	CLOCK_MGMT_DT_DEFINE_OUTPUT(DT_DRV_INST(inst))
 ...
 * devices described in devicetree should use @ref CLOCK_MGMT_DT_GET_OUTPUT.
 * Before using this macro, @ref CLOCK_MGMT_DEFINE_OUTPUT should be used to
 ...
 * @param name Software defined name for this clock output
 */
#define CLOCK_MGMT_GET_OUTPUT(node_id, name)                                   \
	/* We only actually define output objects if runtime clocking is on */ \
	COND_CODE_1(CONFIG_CLOCK_MGMT_RUNTIME, (                               \
	&Z_CLOCK_MGMT_OUTPUT_NAME(Z_CLOCK_OUTPUT_SYMBOL_NAME(node_id, name))), \
 ...
 * CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_NAME should be used for defining the clock
 * output
 * @param dev_node Device node with a clock-outputs property.
 * @param name Name of the clock output
 */
#define CLOCK_MGMT_DT_GET_OUTPUT_BY_NAME(dev_node, name)                       \
	CLOCK_MGMT_GET_OUTPUT(                                                 \
		DT_PHANDLE_BY_IDX(dev_node, clock_outputs,                     \
		DT_CLOCK_OUTPUT_NAME_IDX(dev_node, name)),                     \
		DT_DEP_ORD(dev_node))
 ...
 * Gets a clock output for the system clock node with name @p name in the
 * device's "clock-outputs" property. Before using this macro, @ref
 * CLOCK_MGMT_DT_INST_DEFINE_OUTPUT_BY_NAME should be used for defining the
 * clock output
 * @param inst DT_DRV_COMPAT instance number
 * @param name Name of the clock output
 */
#define CLOCK_MGMT_DT_INST_GET_OUTPUT_BY_NAME(inst, name)                      \
	CLOCK_MGMT_DT_GET_OUTPUT_BY_NAME(DT_DRV_INST(inst), name)
 ...
 * device's "clock-outputs" property. Before using this macro, @ref
 * CLOCK_MGMT_DT_DEFINE_OUTPUT_BY_IDX should be used for defining the clock
 * output
 * @param dev_node Device node with a clock-outputs property.
 * @param idx Index within the "clock-outputs" property
 */
#define CLOCK_MGMT_DT_GET_OUTPUT_BY_IDX(dev_node, idx)                         \
	CLOCK_MGMT_GET_OUTPUT(                                                 \
		DT_PHANDLE_BY_IDX(dev_node, clock_outputs, idx),               \
		DT_DEP_ORD(dev_node))
 ...
 * Gets a clock output for the system clock node with index @p idx in the
 * device's "clock-outputs" property. Before using this macro, @ref
 * CLOCK_MGMT_DT_INST_DEFINE_OUTPUT_BY_IDX should be used for defining the clock
 * output
 * @param inst DT_DRV_COMPAT instance number
 * @param idx Index within the "clock-outputs" property
 */
#define CLOCK_MGMT_DT_INST_GET_OUTPUT_BY_IDX(inst, idx)                        \
	CLOCK_MGMT_DT_GET_OUTPUT_BY_IDX(DT_DRV_INST(inst), idx)
 ...
 * Before using this macro, @ref CLOCK_MGMT_DT_DEFINE_OUTPUT should be used for
 * defining the clock output. Note this is equivalent to
 * CLOCK_MGMT_DT_GET_OUTPUT_BY_IDX(dev_node, 0)
 * @param dev_node Device node with a clock-outputs property.
 */
#define CLOCK_MGMT_DT_GET_OUTPUT(dev_node)                                     \
	CLOCK_MGMT_DT_GET_OUTPUT_BY_IDX(dev_node, 0)
 ...
 * Gets a clock output for device described in devicetree. The output will be
 * retrievd from the first phandle in the node's "clock-outputs" property.
 * Before using this macro, @ref CLOCK_MGMT_DT_INST_DEFINE_OUTPUT should be used
 * for defining the clock output. Note this is equivalent to
 * CLOCK_MGMT_DT_INST_GET_OUTPUT_BY_IDX(inst, 0)
 * @param inst DT_DRV_COMPAT instance number
 */
#define CLOCK_MGMT_DT_INST_GET_OUTPUT(inst)                                    \
	CLOCK_MGMT_DT_GET_OUTPUT(DT_DRV_INST(inst))
 ...
 *     CLOCK_MGMT_DT_GET_STATE(DT_NODELABEL(my_dev), low_power, sleep)
 *     // Get identifier to apply "hsclk_state0" (highspeed clock, active state)
 *     CLOCK_MGMT_DT_GET_STATE(DT_NODELABEL(my_dev), highspeed, active)
 ...
 */
#define CLOCK_MGMT_DT_GET_STATE(dev_id, output_name, state_name)               \
 ...
 * to CLOCK_MGMT_DT_GET_STATE(DT_DRV_INST(inst), output_name, state_name)
 * @param inst DT_DRV_COMPAT instance number
 * @param output_name Name of clock output to read state for
 * @param state_name Name of clock state to get for this clock output
 */
#define CLOCK_MGMT_DT_INST_GET_STATE(inst, output_name, state_name)            \
	CLOCK_MGMT_DT_GET_STATE(DT_DRV_INST(inst), output_name, state_name)
 ...
 */
int clock_mgmt_get_rate(const struct clock_output *clk);
 ...
 * `CONFIG_CLOCK_MGMT_RUNTIME`, existing constraints on the clock will be
 * accounted for when servicing this request. Additionally, if enabled via
 * `CONFIG_CLOCK_MGMT_SET_RATE`, the clock will dynamically request a new rate
 ...
 * retrieved using the @ref CLOCK_MGMT_DT_GET_STATE macro
 ...
 * @return 0 on success
 */
static inline int clock_mgmt_set_callback(const struct clock_output *clk,
...
{
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	const struct clock_mgmt_event event = {
		.type = CLOCK_MGMT_QUERY_RATE_CHANGE,
		.old_rate = 0,
		.new_rate = 0,
	};
