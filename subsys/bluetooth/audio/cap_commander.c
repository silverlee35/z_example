...
{
	struct bt_cap_common_proc *active_proc = bt_cap_common_get_active_proc();
	struct bt_conn *cap_conn = NULL;
	char le_addr[BT_ADDR_LE_STR_LEN];
	uint16_t total_bis_sync;
	uint8_t src_id = 0;
	...
	LOG_DBG("BASS recv state: src_id %u, addr %s, sid %u, sync_state %u", state->src_id,
		le_addr, state->adv_sid, state->pa_sync_state);

	if (state->encrypt_state == BT_BAP_BIG_ENC_STATE_BAD_CODE) {
		LOG_DBG("Encryption state is BT_BAP_BIG_ENC_STATE_BAD_CODE");
		return;
	}
	...
	if (bt_cap_common_conn_in_active_proc(conn) &&
		...
		for (uint8_t i = 0; i < state->num_subgroups; i++) {
			...
			total_bis_sync |= subgroup->bis_sync;
		}
		for (size_t i = 0; i < active_proc->proc_cnt; i++) {
			if (state->src_id == active_proc->proc_param.commander[i]
						     .broadcast_reception_stop.src_id &&
			    conn == active_proc->proc_param.commander[i].conn) {
				src_id = state->src_id;
				cap_conn = conn;
			}
		}

		if (total_bis_sync == 0 && cap_conn == conn) {
			LOG_DBG("Removing source for conn %p", (void *)cap_conn);
			err = bt_bap_broadcast_assistant_rem_src(cap_conn, src_id);
			if (err != 0) {
				LOG_DBG("Failed to perform broadcast reception stop for conn %p: "
					"%d",
					(void *)cap_conn, err);
				bt_cap_common_abort_proc(cap_conn, err);
				cap_commander_proc_complete();
			}
		}
	}
...
static void cap_commander_broadcast_assistant_rem_src_cb(struct bt_conn *conn, int err)
{
	...
	if (!bt_cap_common_proc_is_done()) {
		...
		if (err != 0) {
			LOG_DBG("Failed to perform broadcast reception stop for conn %p: %d",
				(void *)conn, err);
			bt_cap_common_abort_proc(conn, err);
			cap_commander_proc_complete();
		}
