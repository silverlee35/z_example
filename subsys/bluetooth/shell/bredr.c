			       ...
			       struct bt_sdp_client_result *result)
{
	...
	if (result) {
		shell_print(ctx_shell, "SDP HFPHF data@%p (len %u) hint %u from"
			    " remote %s", result->resp_buf,
			    result->resp_buf->len, result->next_record_hint,
			    addr);
		...
		res = bt_sdp_get_features(result->resp_buf, &features);
		if (res < 0) {
			shell_error(ctx_shell, "Error getting HFPHF Features, "
				    "err %d", res);
			goto done;
		}
		shell_print(ctx_shell, "HFPHF Supported Features param 0x%04x",
		      features);
	} else {
		shell_print(ctx_shell, "No SDP HFPHF data from remote %s",
			    addr);
	}
done:
	return BT_SDP_DISCOVER_UUID_CONTINUE;
