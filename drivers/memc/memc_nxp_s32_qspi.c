...
#define SFP_MDAD_NODE(n, i) DT_INST_CHILD(n, sfp_mdad_##i)
...
#define _QSPI_SFP_MDAD_CFG(i, n)							\
	COND_CODE_1(DT_NODE_EXISTS(SFP_MDAD_NODE(n, i)),				\
		({ /* Get configuration from DT if DT node exists */			\
			.SecureAttribute = QSPI_SECURE_ATTRIBUTE(SFP_MDAD_NODE(n, i)),	\
			.MaskType = DT_ENUM_IDX(SFP_MDAD_NODE(n, i), mask_type),	\
			.Valid = true,							\
			.Mask = DT_PROP(SFP_MDAD_NODE(n, i), mask),			\
			.DomainId = DT_PROP(SFP_MDAD_NODE(n, i), domain_id),		\
		},),									\
		({ /* Otherwise mark as invalid configuration (not configured) */	\
			.Valid = false							\
		},))

#define QSPI_SFP_MDAD_CFG(n)								\
	.Tg = {										\
		LISTIFY(QuadSPI_MDAD_COUNT, _QSPI_SFP_MDAD_CFG, (), n)			\
	},
#endif /* QSPI_IP_SFP_ENABLE_MDAD */

#if QSPI_IP_SFP_ENABLE_FRAD
#define SFP_FRAD_NODE(n, i) DT_INST_CHILD(n, sfp_frad_##i)
...
#define _QSPI_SFP_FRAD_CFG(i, n)							\
	COND_CODE_1(DT_NODE_EXISTS(SFP_FRAD_NODE(n, i)),				\
		({ /* Get configuration from DT if DT node exists */			\
			.StartAddress = DT_PROP(SFP_FRAD_NODE(n, i), start_address),	\
			.EndAddress = DT_PROP(SFP_FRAD_NODE(n, i), end_address),	\
			.Valid = true,							\
			.Md0Acp = QSPI_ACP_POLICY(SFP_FRAD_NODE(n, i)),			\
			.Md1Acp = QSPI_ACP_POLICY(SFP_FRAD_NODE(n, i)),			\
			.ExclusiveAccessLock = QSPI_EXCLUSIVE_ACCESS_LOCK(SFP_FRAD_NODE(n, i)),\
			.ExclusiveAccessOwner = DT_PROP(SFP_FRAD_NODE(n, i),		\
							exclusive_access_owner),	\
		},),									\
		({ /* Otherwise mark as invalid configuration (not configured) */	\
			.Valid = false							\
		},))
...
#define QSPI_SFP_FRAD_CFG(n)								\
	.Frad = {									\
		LISTIFY(QuadSPI_FRAD_COUNT, _QSPI_SFP_FRAD_CFG, (), n)			\
	},
