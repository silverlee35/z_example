...
static void
phy_dm8806_gpio_callback(const struct device *dev, struct gpio_callback *cb,
					 uint32_t pins)
{
	ARG_UNUSED(pins);
	struct phy_dm8806_data *drv_data =
		CONTAINER_OF(cb, struct phy_dm8806_data, gpio_cb);
...
static void
phy_dm8806_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p3);
...
int phy_dm8806_init_interrupt(const struct device *dev)
{
	...
	*/
	...
	/* Activate interrupt per Ethernet port by writing "1" to to LNK_EN0~3
...
static int phy_dm8806_reg_read(const struct device *dev, uint16_t reg_addr,
			       uint32_t *data)
{
	int res;
	const struct phy_dm8806_config *cfg = dev->config;
...
static int phy_dm8806_reg_write(const struct device *dev, uint16_t reg_addr,
				uint32_t data)
{
	int res;
	const struct phy_dm8806_config *cfg = dev->config;
