...
#define DT_DRV_COMPAT nxp_s32_psi5_controller
...
static int psi5_nxp_s32_start(const struct device *dev, uint8_t channel_id)
{
	const struct psi5_nxp_s32_config *config = dev->config;
	struct psi5_nxp_s32_data *data = dev->data;
	struct psi5_nxp_s32_channel_data *channel_data = &data->channel_data[channel_id];
	int err;
	...
	err = Psi5_Ip_SetChannelSync(config->ctrl_inst, channel_id, true);

	if (err) {
		LOG_ERR("Failed to start PSI5 %d channel %d", config->ctrl_inst, channel_id);
		return -EIO;
	}
...
static int psi5_nxp_s32_stop(const struct device *dev, uint8_t channel_id)
{
	const struct psi5_nxp_s32_config *config = dev->config;
	struct psi5_nxp_s32_data *data = dev->data;
	struct psi5_nxp_s32_channel_data *channel_data = &data->channel_data[channel_id];
	...
	err = Psi5_Ip_SetChannelSync(config->ctrl_inst, channel_id, false);

	if (err) {
		LOG_ERR("Failed to stop PSI5 %d channel %d", config->ctrl_inst, channel_id);
		return -EIO;
	}
...
static int psi5_nxp_s32_send(const struct device *dev, uint8_t channel_id, uint64_t psi5_data,
			     k_timeout_t timeout, psi5_tx_callback_t callback, void *user_data)
{
	...
	err = Psi5_Ip_Transmit(config->ctrl_inst, channel_id, psi5_data);
	if (err) {
	...
	while (!Psi5_Ip_GetTransmissionStatus(config->ctrl_inst, channel_id)) {
...
static void psi5_nxp_s32_add_rx_callback(const struct device *dev, uint8_t channel_id,
...
#define PSI5_NXP_S32_CHANNEL_TX_SEM_INIT(node_id)                                                  \
	k_sem_init(&data->channel_data[DT_REG_ADDR(node_id)].tx_sem, 1, 1);

#define PSI5_NXP_S32_INIT(n)                                                                       \
	...
	{                                                                                          \
		...
		int err = 0;                                                                       \
                                                                                                   \
		DT_INST_FOREACH_CHILD_STATUS_OKAY(n, PSI5_NXP_S32_CHANNEL_TX_SEM_INIT)             \
                                                                                                   \
...
DT_INST_FOREACH_STATUS_OKAY(PSI5_NXP_S32_INIT)

/*
 * The following definitions is defined for the initial configuration that used for Psi5_Ip_Init()
 */
...
#define PSI5_NXP_S32_CHANNEL_CALLBACK(node_id)                                                     \
	...
	{                                                                                          \
		...
		if (event.Psi5_Psi5MessageReceived) {                                              \
			Psi5_Ip_GetPsi5Frame(config->ctrl_inst, channel_id, &ip_frame);            \
                                                                                                   \
		...
		if (event.Psi5_Psi5MessageOverwrite) {                                             \
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, channel_id, NULL,                       \
		...
		if (event.Psi5_Psi5MessageErrorsPresent) {                                         \
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, channel_id, NULL, PSI5_STATE_MSG_ERR,   \
						      rx_callback->user_data);                     \
			}                                                                          \
		}                                                                                  \
		if (event.Psi5_SmcMessageReceived) {                                               \
			Psi5_Ip_GetSmcFrame(config->ctrl_inst, channel_id, &ip_smc_frame);         \
                                                                                                   \
			if (ip_smc_frame.C) {                                                      \
...
/* The structure configuration for all PSI5 controllers */
...
/* Initialize all PSI5 controllers */
static int psi5_nxp_s32_ctrl_init(void)
{
...
SYS_INIT(psi5_nxp_s32_ctrl_init, POST_KERNEL, CONFIG_PSI5_INIT_PRIORITY);
