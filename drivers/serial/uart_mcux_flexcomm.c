...
struct mcux_flexcomm_config {
	...
	clock_mgmt_state_t clock_state;
#else
...
#ifdef CONFIG_CLOCK_MGMT
	clock_freq = clock_mgmt_get_rate(config->clock_output);
#else
...
int uart_mcux_flexcomm_clock_cb(const struct clock_mgmt_event *ev, const void *data)
{
	const struct device *uart_dev = data;
	const struct mcux_flexcomm_config *config = uart_dev->config;

	if (ev->type == CLOCK_MGMT_PRE_RATE_CHANGE) {
		/* Deinit USART */
		USART_Deinit(config->base);
	} else if (ev->type == CLOCK_MGMT_POST_RATE_CHANGE) {
		/* Reconfigure USART */
...
#ifdef CONFIG_CLOCK_MGMT
	clock_freq = clock_mgmt_apply_state(config->clock_output,
					    config->clock_state);
	mcux_flexcomm_uart_setup(dev, clock_freq);
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	clock_mgmt_set_callback(config->clock_output,
...
#ifdef CONFIG_CLOCK_MGMT
#define UART_MCUX_FLEXCOMM_CLK_DEFINE(n) CLOCK_MGMT_DT_INST_DEFINE_OUTPUT(n)
#define UART_MCUX_FLEXCOMM_CLK_INIT(n)						\
	.clock_output = CLOCK_MGMT_DT_INST_GET_OUTPUT(n),                       \
	.clock_state = CLOCK_MGMT_DT_INST_GET_STATE(n, default, default),
#else
