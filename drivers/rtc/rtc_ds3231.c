...
static int ds3231_set_time(const struct device *dev, const struct rtc_time *tm)
{
	...
	err = i2c_burst_write_dt(&config->i2c_bus, DS3231_REG_SECONDS, regs, sizeof(regs));
...
static int ds3231_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	...
	err = i2c_burst_read_dt(&config->i2c_bus, DS3231_REG_SECONDS, regs, sizeof(regs));
	if (err != 0) {
		goto unlock;
	}
...
static int ds3231_init(const struct device *dev)
{
	...
	/* Make clock halt = 0 */
	err = i2c_reg_write_byte_dt(&config->i2c_bus, DS3231_REG_SECONDS, 0x00);
	if (err < 0) {
