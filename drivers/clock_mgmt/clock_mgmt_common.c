...
LOG_MODULE_REGISTER(clock_mgmt, CONFIG_CLOCK_MGMT_LOG_LEVEL);
...
struct clock_output_data {
	...
	struct clock_mgmt_rate_req *combined_req;
#endif
};
 ...
 */
static void clock_add_constraint(struct clock_mgmt_rate_req *current,
				 const struct clock_mgmt_rate_req *new)
{
			...
			const struct clock_mgmt_rate_req *req)
{
 ...
 #ifdef CONFIG_CLOCK_MGMT_RUNTIME
	struct clock_mgmt_rate_req new_req;
	/*
	 ...
	 * request directly
	 */
	combined_req = (struct clock_mgmt_rate_req *)req;
#endif

#ifdef CONFIG_CLOCK_MGMT_CLK_NAME
 ...
 * retrieved using the @ref DT_CLOCK_MGMT_STATE macro
			   ...
			   clock_mgmt_state_t state)
{
...
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	struct clock_mgmt_rate_req temp;
	...
	if (clk_state->locking) {
		...
		const struct clock_mgmt_rate_req constraint = {
...
#if defined(CONFIG_CLOCK_MGMT_RUNTIME)
					...
					const struct clock_mgmt_event *event)
{
	...
	if (event->type == CLOCK_MGMT_QUERY_RATE_CHANGE) {
...
const struct clock_mgmt_output_api clock_output_api = {
#if defined(CONFIG_CLOCK_MGMT_RUNTIME)
...
#define CLOCK_SETTINGS_GET(node, prop, idx)                                    \
	{                                                                      \
		...
		.clock_config_data = Z_CLOCK_MGMT_CLK_DATA_GET(node, prop,     \
							       idx),           \
	}
...
#define CLOCK_STATE_DEFINE(node)                                               \
	...
	DT_FOREACH_PROP_ELEM(node, clocks, Z_CLOCK_MGMT_CLK_DATA_DEFINE);))    \
...
#define CLOCK_OUTPUT_RUNTIME_DEFINE(inst)                                      \
	...
	struct clock_mgmt_rate_req combined_req_##inst = {                     \
		.min_freq = 0,                                                 \
		.max_freq = INT32_MAX,                                         \
	};
...
#define CLOCK_OUTPUT_DEFINE(inst)                                              \
	...
	CLOCK_DT_INST_DEFINE(inst,                                             \
			     &CONCAT(clock_output_, DT_INST_DEP_ORD(inst)),    \
			     (struct clock_mgmt_driver_api *)&clock_output_api);
...
#endif /* CONFIG_CLOCK_MGMT_RUNTIME */
