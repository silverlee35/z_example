...
CLOCK_MGMT_DT_INST_DEFINE_OUTPUT(0);

static const struct clock_output *cpu_clock = CLOCK_MGMT_DT_INST_GET_OUTPUT(0);
...
static int core_clock_change_cb(const struct clock_mgmt_event *ev, const void *data)
{
	ARG_UNUSED(data);
...
{
	clock_mgmt_state_t default_state =
		CLOCK_MGMT_DT_INST_GET_STATE(0, default, default);
	int new_rate;
...
#ifdef CONFIG_CLOCK_MGMT_RUNTIME
	clock_mgmt_set_callback(cpu_clock, core_clock_change_cb, NULL);
#else
	change_core_clock(SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
#endif
	new_rate = clock_mgmt_apply_state(cpu_clock, default_state);
