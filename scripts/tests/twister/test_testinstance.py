...
def test_check_build_or_run(
    ...
    run = testinstance.check_runnable(slow, device_testing, platform_sim, fixture)
    _, r = expected
    assert run == r
...
def test_testinstance_check_runnable(
    ...
    with mock.patch('os.name', os_name), \
        ...
        res = testinstance.check_runnable(enable_slow, filter, fixtures,  platform_sim, hardware_map)
